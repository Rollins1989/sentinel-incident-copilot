...
class AnswerEngine:
    ...
    def answer(self, question: str, prompt_version: str | None = None) -> AnswerResult:
        ...
        if decision.should_refuse:
            ...
            result = AnswerResult(REFUSAL_MESSAGE, True, decision.reason, [], True, [], decision.top_score, trace_id, retrieved, latencies)
        ...
        groundedness = check_groundedness(llm_response.text, [chunk.chunk_id for chunk in retrieved])
        ...
        citations = [
            Citation(chunk.chunk_id, chunk.metadata["title"], chunk.metadata["section"], chunk.metadata["source_path"], chunk.metadata["doc_type"])
            for chunk in retrieved if chunk.chunk_id in groundedness.cited_chunk_ids
        ]
        result = AnswerResult(
            safe_text,
            is_explicit_refusal,
            refusal_reason,
            citations if not is_explicit_refusal else [],
            groundedness.is_grounded or is_explicit_refusal,
            groundedness.invalid_citations,
            decision.top_score,
            trace_id,
            retrieved,
            latencies,
        )
        self._log_event(question, result, prompt_version, llm_response.input_tokens, llm_response.output_tokens, prompt_hash)
        return result

    def _log_event(self, question, result, prompt_version, input_tokens, output_tokens, prompt_hash=""):
        record_query_event(
            trace_id=result.trace_id, question=question,
            ...
            confidence=result.confidence, refused=int(result.refused),
