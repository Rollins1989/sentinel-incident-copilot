...
REFUSAL_MESSAGE = (
    ...
    "to answer this confidently. Escalate to a human on-call engineer or add a runbook "
    "for this scenario."
)

DOMAIN_TERMS = {
    "api", "service", "server", "database", "db", "pod", "pods", "kubernetes",
    "deploy", "deployment", "crashloopbackoff", "incident", "outage", "error",
    "errors", "oomkilled", "502", "503", "latency", "traffic", "request",
    "requests", "retry", "retries", "backoff", "webhook", "payment", "provider",
    "billing", "checkout", "notification", "search", "connection", "connections",
    "pool", "authentication", "authenticate", "token", "api-key", "rate", "limit",
    "remediation", "diagnostic", "root", "cause", "postmortem", "runbook",
    "memory", "restart", "restarting",
}
OUT_OF_SCOPE_TERMS = {
    "pto", "vacation", "leave", "holiday", "salary", "payroll", "hiring",
    "interview", "performance", "promotion", "hr",
}
...
def evaluate_refusal(
    retrieved: list[RetrievedChunk], question: str | None = None
) -> RefusalDecision:
    if not retrieved:
